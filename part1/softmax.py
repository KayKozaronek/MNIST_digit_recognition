...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    # Compute matrix theta* X.T (row = category, column = example)
    R = theta.dot(X.T) / temp_parameter
    
    # Compute c for numerical stability (c = vector 1xn)
    c = np.max(R, axis=0)
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    # mapping 
    map =  sparse.coo_matrix(([1]*n, (Y, range(n))), shape = (k,n)).toarray()
    # Loss term
    loss = - 1 / n * np.sum(H_log[map == 1])
